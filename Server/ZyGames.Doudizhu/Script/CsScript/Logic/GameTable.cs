                        ...
                        //{
                            ...
                            foreach (var pos in table.Positions)
                            {
                                if (pos.UserId == 0)
                                {
                                    ...
                                    try
                                    {
                                        var nameList = AIConfig.NickName;// scope.AIConfig.getConfig("nickName");
                                        if (nameList != null && nameList.Length > 0)
                                        {
                                            for (int i = 0; i < 5; i++)
                                            {
                                                int index = RandomUtils.GetRandom(0, nameList.Length);
                                    ...
                                    var headList = AIConfig.Head;// scope.AIConfig.getConfig("head");
                                        if (headList != null && headList.Length > 0)
                                        {
                                            int index = RandomUtils.GetRandom(0, headList.Length);
                                            head = headList[index];
                                        }
                                    }
                                    catch (Exception ex)
                        ...
                        if (pos != null && pos.IsAI)
                        {
                            ...
                            //{
                                ...
                                try
                                {
                                    ...
                                    var myInstance = new CardAILogic(table.RoomId, table.TableId, pos.Id);
                                    iscall = myInstance.CheckCall();
                                }
                                catch (Exception e)
                                {
                        ...
                        if (pos != null && pos.IsAI && pos.CardData.Count > 0)
                        {
                            try
                            {
                                ...
                                //{
                                    ...
                                    if (outList != null)
                                    {
                                        int[] cardArr = new int[outList.Count];
